/*
Author: Najmul Huda
Problem solve : 2025-09-19 08:21:30
*/
#include <bits/stdc++.h>
using namespace std;
...
{   
    ...
    fr(i,0,n-1,1){
    unordered_map<ll,ll>primeFactor = prime_factorization(arr[i]); 
    vl div;
    div =divisors(arr[i]);
    ll gpf=spf[arr[i]];
    ll totalPF =0;
    for(auto [gp,_]:primeFactor) gpf=max(gpf,gp),totalPF+=_;
    ll divSum =0;
    for(auto v:div) divSum+=v;
    cout<<spf[arr[i]]<<" "<<gpf<<" "<<primeFactor.size()<<" "<<totalPF<<" "<<div.size()<<" "<<divSum<<endl;
    }
}
